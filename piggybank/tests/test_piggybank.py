...
class TestStandardTransaction:
    ...
    async def make_and_spend_piggybank(self, network, alice, bob, CONTRIBUTION_AMOUNT):
        ...
        piggybank_coin = await alice.launch_smart_coin(create_piggybank_puzzle(uint64(1000000), bob.puzzle_hash))
        ...
        try:
            ...
            filtered_result = list(filter(
                lambda addition:
                    ...
                    (addition.puzzle_hash == create_piggybank_puzzle(1000000, bob.puzzle_hash).get_tree_hash())
            ,result["additions"]))
    ...
    async def test_piggybank_completion(self, setup):
        network, alice, bob = setup
        try:
            result = await self.make_and_spend_piggybank(network, alice, bob, 1000000)
